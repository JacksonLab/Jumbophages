...
if (metaspacers==F){  
    # Join the spacer/repeat assignment information
      spacer.details<-read_delim("output/typeIE_spacers.txt",col_names=c("GCF","CRISPR","spacer.seq","spacer","spacer.ID","CRISPR.score","CRISPR.type","repeat.seq","repeat.length","ID"),delim="\t")
  # Join
  ...
  # Read in strain names
    strain.names<-read_delim("output/GCF_strain_names_typeIE.txt",delim="\t",col_names=c("assembly","strain"))%>%
...
if (IMG.VR==T){    
  ...
  # Output
    write_xlsx(classified.results,paste0("output/Processed_IE_",filename,".xlsx"))
    
return (classified.results)
}
  
# Data were first processed as <> 200 kb, but these are later merged and filtered into <> 200 kb.
hits_Genbank_gb150kb<-Process_Matches("search_results_typeIE_spacers_unique_Refseq_genbank_caudovirales_150kb.fasta",F,F)%>%
  mutate(system="TypeIE",target.database="gb>150kb")
hits_Genbank_gb150kb_shuffled<-Process_Matches("search_results_typeIE_spacers_unique_Refseq_caudovirales_150kb.fasta_shuffled",F,F)%>%
  mutate(system="TypeIE",target.database="gb>150kb")

hits_Genbank_gb_sub150kb<-Process_Matches("search_results_typeIE_spacers_unique_Refseq_caudovirales_sub150kb.fasta",F,F)%>%
  mutate(system="TypeIE",target.database="gb<150kb")
hits_Genbank_gb_sub150kb_shuffled<-Process_Matches("search_results_typeIE_spacers_unique_Refseq_caudovirales_sub150kb.fasta_shuffled",F,F)%>%
  mutate(system="TypeIE",target.database="gb<150kb")

hits_Genbank_IMGVR150kb<-Process_Matches("search_results_typeIE_spacers_unique_Refseq_IMGVR_150kb.fasta",F,T)%>%
  mutate(system="TypeIE",target.database="IMGVR>150kb")
hits_Genbank_IMGVR150kb_shuffled<-Process_Matches("search_results_typeIE_spacers_unique_Refseq_IMGVR_150kb.fasta_shuffled",F,T)%>%
  mutate(system="TypeIE",target.database="IMGVR>150kb")

hits_Genbank_IMGVRsub150kb<-Process_Matches("search_results_typeIE_spacers_unique_Refseq_IMGVR_sub150kb.fasta",F,T)%>%
  mutate(system="TypeIE",target.database="IMGVR<150kb")
hits_Genbank_IMGVRsub150kb_shuffled<-Process_Matches("search_results_typeIE_spacers_unique_Refseq_IMGVR_sub150kb.fasta_shuffled",F,T)%>%
  mutate(system="TypeIE",target.database="IMGVR<150kb")
...
# Output the data - use another script to analyse all types together...
  write_xlsx(all_hits,"output/hits_typeIE.xlsx")
